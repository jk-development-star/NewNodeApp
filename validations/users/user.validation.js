...
const stringPassswordError = "Password length must be 8 to 20 characters long and password must contains a combination of uppercase, lowercase letter, number, and a special character are required."
...
const userSchemaValidate = Joi.object({
    ...
    phone: Joi.string().trim().regex(mobilePattern).rule({ message: stringMobileError }).length(10).required(),
    role: Joi.string().trim().required(),
    password: Joi.string().trim().regex(passwordPattern).rule({ message: stringPassswordError }).required().label('Password'),
