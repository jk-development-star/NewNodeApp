...
const leadCreate = async (req, res) => {
    ...
    res.render("newViews/leads/create", {
        usersList, layout: true, title: "Add New Lead"
    });
}

const getAllLead = async (req, res) => {
    try {
        await leadDriver.getAllLeads().then(leads => {
            if (!leads) {
                req.flash('error', leadMessage.MESSAGE_NO_LEAD_FOUND)
                return res.render('newViews/leads/index', { title: "Lead List", layout: true })
            } else {
                console.log(leads);
                return res.render('newViews/leads/index', { leads, title: "Lead List", layout: true })
            }
        }).catch(error => {
...
const storeLead = async (req, res) => {
    ...
    if (error) {
        ...
        return res.render('newViews/leads/create', { title: 'Add New Lead', layout: true });
    }
    try {
        var { generatedBy, lead_id, ...data } = value
        data['generatedBy'] = req.session.user._id
        data['lead_id'] = Date.now()
        await leadDriver.leadCreate(data).then((lead) => {
            if (lead)
                req.flash('success', leadMessage.MESSAGE_SUCCESS_CREATE_LEAD)
            return res.redirect(301, '/leads')

        }).catch(error => {
            req.flash('error', error)
            return res.render('newViews/leads/create', { title: 'Add New Lead', layout: 'layout' });
        })
    } catch (error) {
        req.flash('error', leadMessage.MESSAGE_INTERNAL_SERVER_ERROR)
        return res.render('newViews/leads/create', { title: 'Add New Lead', layout: 'layout' });
