...
const userCreate = (req, res) => {
    res.render("newViews/users/create", {
        layout: true, title: "Create New User"
    });
}
...
const storeUser = async (req, res) => {
    ...
    if (error) {
        ...
        return res.render('newViews/users/create', { title: 'Create New User', layout: 'layout' });
    }
    ...
    //create new user
    await userDriver.createUser(data).then((user) => {
        if (user)
...
const userList = async (req, res) => {
    await userDriver.getAllUsers().then(users => {
        if (users) return res.render('newViews/users/index', { users, title: 'Users List', layout: true })
        else req.flash('error', message.MESSAGE_NO_USER_FOUND)
        return res.render('newViews/users/index', { title: 'Users List', layout: true })

    }).catch(err => {
        userLogger.error('Error in Listing User', { status: '500', message: err });
        req.flash('error', err)
        return res.render('newViews/users/index', { title: 'Users List', layout: true })
    });
...
const deleteAUser = async (req, res) => {
    try {
        ...
        //Logging the error
        userLogger.error('Error in deleting User', { status: '500', message: error });
        req.flash('error', message.MESSAGE_INTERNAL_SERVER_ERROR)
        return res.render('newViews/users/index', { title: 'Users List', layout: true })
    }
}
...
const userEditView = async (req, res) => {
    try {
        ...
        await userDriver.userEdit(req.params.id).then(user => {
            if (user) return res.render('newViews/users/edit', { user, title: 'Edit User', layout: true });
        })
...
const userView = async (req, res) => {
    try {
        ...
        await userDriver.userEdit(req.params.id).then(userProfile => {
            ...
            if (userProfile) return res.render('newViews/users/view', { userProfile, title: 'User Profile', layout: true });
        })
...
const updateUser = async (req, res) => {
    try {
        await userDriver.userUpdate(req.params.id, req.body).then(user => {
            if (!user) {
                req.flash('error', message.MESSAGE_NO_USER_FOUND)
                return res.render('newViews/users/edit', { title: 'Edit User', layout: true })
            } else {
